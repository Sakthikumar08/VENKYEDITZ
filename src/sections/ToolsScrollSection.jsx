...
const ToolsScrollSection = () => {
  ...
  return (
    <section className="relative py-20 overflow-hidden bg-gradient-to-b from-slate-900/30 to-slate-900/10 border-t border-b border-cyan-500/10">
      ...
      <div className="container mx-auto px-4 relative z-20">
        <motion.h2
          className="text-3xl md:text-4xl font-bold mb-12 text-center text-cyan-400 font-mono tracking-wider"
          ...
          <div className="flex gap-6 pb-4 min-w-max px-4">
            {[...tools, ...tools, ...tools, ...tools].map((tool, index) => (
              <motion.div key={index} className="flex-shrink-0">
                <motion.div
                  className="relative w-36 h-44 group overflow-hidden"
                ...
                >
                  ...
                  <div className="absolute inset-0 bg-gradient-to-br from-slate-800/60 to-slate-900/80 backdrop-blur-md border-l-2 border-t-2 border-cyan-500/50 group-hover:border-cyan-400 transition-all">
                    {/* Corner decorations */}
                    <div className="absolute -top-1 -left-1 w-4 h-4 border-l-2 border-t-2 border-cyan-400/60 group-hover:border-cyan-400 transition-all"></div>
                    <div className="absolute -top-1 -right-1 w-4 h-4 border-r-2 border-t-2 border-orange-400/40 group-hover:border-orange-400/80 transition-all"></div>
                    <div className="absolute -bottom-1 -left-1 w-4 h-4 border-l-2 border-b-2 border-cyan-400/40 group-hover:border-cyan-400/80 transition-all"></div>
                    <div className="absolute -bottom-1 -right-1 w-4 h-4 border-r-2 border-b-2 border-orange-400/60 group-hover:border-orange-400 transition-all"></div>

                    {/* Inner content */}
                    <div className="w-full h-full flex flex-col items-center justify-center gap-3 p-4">
                      <motion.div
                        className="text-3xl font-mono font-bold bg-gradient-to-br from-cyan-400 to-orange-400 bg-clip-text text-transparent"
                        animate={{ rotate: 360 }}
        ...
        <div className="flex justify-center mt-8 gap-2">
          {[...Array(4)].map((_, i) => (
            <motion.div
              key={i}
              className="w-2 h-2 bg-gradient-to-r from-cyan-500 to-orange-500"
